...
export function handler(event: any, context: any, callback: any) {
  ...
  const getAttachments = (data: any) => data.attachments.filter((a: any) => a.contentDisposition === 'attachment')
    ...
    [
      function getRawEmail(next: any) {
        console.log(
          `Getting raw email ${
            ses.mail.messageId
          } from ${INBOX_S3_BUCKET_NAME} for ${origOrg}`
        )
        s3.getObject({
